...
if grep -Eq -- 'WEB_ZIP_|omarchy-bible-search-setup' "$BIN"; then
  fail 'CLI still has a WEBP download path'
fi
...
setup_output="$($BIN setup)"
assert_contains "$setup_output" 'Bundled WEBP corpus is ready'

missing_root="$(mktemp -d)"
mkdir -p "$missing_root/plugin/bin"
cp -- "$BIN" "$missing_root/plugin/bin/omarchy-bible-search"
missing_err=""
if missing_err="$("$missing_root/plugin/bin/omarchy-bible-search" setup 2>&1)"; then
  fail 'setup succeeded without a bundled corpus'
fi
assert_contains "$missing_err" 'no bundled WEBP corpus'
rm -rf -- "$missing_root"
